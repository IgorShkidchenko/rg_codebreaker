...
module ConsoleTexts
  ...
  GREETING_MSG = "Hello, lets play the 'Codebreaker' game".green
  WHAT_NAME_MSG = "What is your name?\nEnter ".green + 'random'.pink + ' to auto-create your name'.green
  ZERO_HINTS_MSG = "You don't have any hints".red

  WIN_MSG = "You win\n-Enter ".green + 'yes'.pink + ' if you want to save your progress'.green
  LOSE_MSG = 'Game over, you lose if you want you can start a new game'.red

  EMPTY_DB_MSG = 'You are the first one'.green
  ...
  def sort_db(db)
    db.sort_by { |user| [user.all_attempts, -user.attempts, -user.hints] }
  end

  def show_db(db)
    puts SHOW_DB_MSG
    position = 0
    sort_db(db).each do |user|
      position += 1
